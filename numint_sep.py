...
def eval_rho(mol, ao, dm, non0tab=None, xctype='LDA', hermi=0, verbose=None):
    ...
    if xctype == 'LDA' or xctype == 'HF':
        ...
        for i in range(1, 4):
            ...
            #rho[i] += np.einsum('pi,pi->p', c1, ao[0])
    else: # meta-GGA
        ...
        rho[5] = 0
        for i in range(1, 4):
            #:rho[i] = np.einsum('pi,pi->p', c0, ao[i]) * 2 # *2 for +c.c.
...
def _vv10nlc(rho,coords,vvrho,vvweight,vvcoords,nlc_pars):
    ...
    #output
    exc=np.zeros(rho[0,:].size)
    vxc=np.zeros([2,rho[0,:].size])
    ...
    #threshed output
    excthresh=np.zeros(R.size)
    vxcthresh=np.zeros([2,R.size])
...
def eval_mat(mol, ao, weight, rho, vxc,
    ...
    if xctype == 'LDA' or xctype == 'HF':
        if not isinstance(vxc, np.ndarray) or vxc.ndim == 2:
            vrho = vxc[0]
        else:
            vrho = vxc
        ...
        #wv = weight * vsigma * 2
        #aow  = np.einsum('pi,p->pi', ao[1], rho[1]*wv)
        #aow += np.einsum('pi,p->pi', ao[2], rho[2]*wv)
        #aow += np.einsum('pi,p->pi', ao[3], rho[3]*wv)
...
# JCP, 112, 7002
    if xctype == 'MGGA':
        ...
        if vlapl is None:
            ...
            XX, YY, ZZ = 4, 7, 9
            ao2 = ao[XX] + ao[YY] + ao[ZZ]
            #:aow = np.einsum('pi,p->pi', ao2, .5 * weight * vlapl, out=aow)
...
def _dot_ao_ao(mol, ao1, ao2, non0tab, shls_slice, ao_loc, hermi=0):
    ...
    if ao1.dtype == ao2.dtype == np.double:
        fn = libdft.VXCdot_ao_ao
    else:
        fn = libdft.VXCzdot_ao_ao
        ao1 = np.asarray(ao1, np.complex128)
        ao2 = np.asarray(ao2, np.complex128)
    ...
    vv = np.empty((nao,nao), dtype=ao1.dtype)
...
def _dot_ao_dm(mol, ao, dm, non0tab, shls_slice, ao_loc, out=None):
    ...
    if not ao.flags.f_contiguous:
        ao = lib.transpose(ao)
    if ao.dtype == dm.dtype == np.double:
        fn = libdft.VXCdot_ao_dm
    else:
        fn = libdft.VXCzdot_ao_dm
        ao = np.asarray(ao, np.complex128)
        dm = np.asarray(dm, np.complex128)
    ...
    vm = np.ndarray((ngrids,dm.shape[1]), dtype=ao.dtype, order='F', buffer=out)
...
def _scale_ao(ao, wv, out=None):
    ...
    aow = np.ndarray((nao,ngrids), dtype=ao.dtype, buffer=out).T

    if not ao.flags.c_contiguous:
        aow = np.einsum('nip,np->pi', ao, wv)
    elif aow.dtype == np.double:
...
def _contract_rho(bra, ket):
    #:rho  = np.einsum('pi,pi->p', bra.real, ket.real)
    #:rho += np.einsum('pi,pi->p', bra.imag, ket.imag)
    bra = bra.T
    ket = ket.T
    ...
    if not (bra.flags.c_contiguous and ket.flags.c_contiguous):
    ...
    elif bra.dtype == np.double and ket.dtype == np.double:
        libdft.VXC_dcontract_rho(rho.ctypes.data_as(ctypes.c_void_p),
                                 ...
                                 ctypes.c_int(nao), ctypes.c_int(ngrids))
    elif bra.dtype == np.complex128 and ket.dtype == np.complex128:
...
def nr_rks_sep(ni, mol, grids, xc_code, dms, relativity=0, hermi=0,
           max_memory=2000, verbose=None):
    ...
    for ao, mask, weight, coords \
        ...
        tot_blksize += weight.size
            ...
            #    wv = _rks_gga_wv0(rho, vxc, np.hstack(weight))
...
# ref eval_mat function
    ...
    if nset == 1:
        #nelec = nelec[0]
        excsum = excsum[0]
        excsum_sep = excsum_sep[0]
        #vmat = vmat.reshape(nao,nao)
    #with open("1.txt",'w') as out:
    #    logger.log(out, 'exc_sep', excsum_sep[0])
    #    logger.slog(out, 'exc %f', excsum)
    return nelec, excsum, vmat, excsum_sep
...
def nr_uks(ni, mol, grids, xc_code, dms, relativity=0, hermi=0,
    ...
    if xctype == 'NLC':
        dms_sf = dms[0] + dms[1]
        nelec, excsum, vmat = dft.numint.nr_rks(ni, mol, grids, xc_code, dms_sf, relativity, hermi,
                                     max_memory, verbose)
...
def _format_uks_dm(dms):
    ...
    if getattr(dms, 'mo_coeff', None) is not None:
        ...
        if mo_coeff[0].ndim < dma.ndim: # handle ROKS
            mo_occa = np.array(mo_occ> 0, dtype=np.double)
            mo_occb = np.array(mo_occ==2, dtype=np.double)
    ...
    if xctype == 'LDA':
        ...
        for ao, mask, weight, coords \
            ...
            if rho0 is None:
                ...
                rho = np.asarray(rho0[:,ip:ip+ngrid], order='C')
...
def nr_rks_fxc_st(ni, mol, grids, xc_code, dm0, dms_alpha, relativity=0, singlet=True,
        ...
        for ao, mask, weight, coords \
            ...
            if singlet:
                ...
                if 0:
                    ...
                    assert(np.linalg.norm(fxc_test[0]*2-frho) < 1e-4)
            else:
                frho = u_u - u_d

            for i in range(nset):
                rho1 = make_rho(i, ao, mask, 'LDA')
                #:aow = np.einsum('pi,p->pi', ao, weight*frho*rho1, out=aow)
    ...
    if isinstance(dms_alpha, np.ndarray) and dms_alpha.ndim == 2:
        vmat = vmat[0]
    return vmat
...
def _rks_gga_wv1(rho0, rho1, vxc, fxc, weight):
    ...
    sigma1 = np.einsum('xi,xi->i', rho0[1:4], rho1[1:4])
    ngrid = vgamma.size
    wv = np.empty((4,ngrid))
    wv[0]  = frho * rho1[0]
...
def _rks_gga_wv2(rho0, rho1, fxc, kxc, weight):
    ...
    sigma1 = np.einsum('xi,xi->i', rho0[1:], rho1[1:])
    r1r1 = rho1[0]**2
    s1s1 = sigma1**2
    r1s1 = rho1[0] * sigma1
    sigma2 = np.einsum('xi,xi->i', rho1[1:], rho1[1:])
    ngrid = frrr.size
    wv = np.empty((4,ngrid))
    wv[0]  = frrr * r1r1
    wv[0] += 4 * frrg * r1s1
    ...
    if xctype == 'LDA':
        ...
        for ao, mask, weight, coords \
            ...
            for i in range(nset):
                ...
                wv *= weight
                #:aow = np.einsum('pi,p->pi', ao, wv, out=aow)
                aow = _scale_ao(ao, wv, out=aow)
...
def _uks_gga_wv1(rho0, rho1, vxc, fxc, weight):
    ...
    rho0a, rho0b = rho0
    rho1a, rho1b = rho1
    a0a1 = np.einsum('xi,xi->i', rho0a[1:4], rho1a[1:4])
    a0b1 = np.einsum('xi,xi->i', rho0a[1:4], rho1b[1:4])
    b0a1 = np.einsum('xi,xi->i', rho0b[1:4], rho1a[1:4])
    b0b1 = np.einsum('xi,xi->i', rho0b[1:4], rho1b[1:4])
...
def _uks_gga_wv2(rho0, rho1, fxc, kxc, weight):
    ...
    b0a1 = np.einsum('xi,xi->i', rho0b[1:4], rho1a[1:4])
    b0b1 = np.einsum('xi,xi->i', rho0b[1:4], rho1b[1:4])
    a1a1 = np.einsum('xi,xi->i', rho1a[1:4], rho1a[1:4])
    a1b1 = np.einsum('xi,xi->i', rho1a[1:4], rho1b[1:4])
    b1a1 = a1b1
    b1b1 = np.einsum('xi,xi->i', rho1b[1:4], rho1b[1:4])
    a0a1_a0a1 = np.einsum('i,i->i', a0a1, a0a1)
    a0a1_a0b1 = np.einsum('i,i->i', a0a1, a0b1)
    a0a1_b0a1 = np.einsum('i,i->i', a0a1, b0a1)
    a0a1_b0b1 = np.einsum('i,i->i', a0a1, b0b1)
    a0b1_a0a1 = a0a1_a0b1
    a0b1_a0b1 = np.einsum('i,i->i', a0b1, a0b1)
    a0b1_b0a1 = np.einsum('i,i->i', a0b1, b0a1)
    a0b1_b0b1 = np.einsum('i,i->i', a0b1, b0b1)
    b0a1_a0a1 = a0a1_b0a1
    b0a1_a0b1 = a0b1_b0a1
    b0a1_b0a1 = np.einsum('i,i->i', b0a1, b0a1)
    b0a1_b0b1 = np.einsum('i,i->i', b0a1, b0b1)
    b0b1_a0a1 = a0a1_b0b1
    b0b1_a0b1 = a0b1_b0b1
    b0b1_b0a1 = b0a1_b0b1
    b0b1_b0b1 = np.einsum('i,i->i', b0b1, b0b1)

    wva = np.zeros((4,ngrid))
    wva[0] += np.einsum('i,i,i->i', u_u_u, rho1a[0], rho1a[0])
    wva[0] += np.einsum('i,i,i->i', u_u_d, rho1a[0], rho1b[0]) * 2
    wva[0] += np.einsum('i,i,i->i', u_d_d, rho1b[0], rho1b[0])
    wva[0] += np.einsum('i,i->i', u_uu, a1a1) * 2
    wva[0] += np.einsum('i,i->i', u_ud, a1b1) * 2
    wva[0] += np.einsum('i,i->i', u_dd, b1b1) * 2
    wva[1:] += np.einsum('i,i,xi->xi', u_uu, rho1a[0], rho1a[1:]) * 4
    wva[1:] += np.einsum('i,i,xi->xi', d_uu, rho1b[0], rho1a[1:]) * 4
    wva[1:] += np.einsum('i,i,xi->xi', u_ud, rho1a[0], rho1b[1:]) * 2
    wva[1:] += np.einsum('i,i,xi->xi', d_ud, rho1b[0], rho1b[1:]) * 2
    wva[1:] += np.einsum('i,i,xi->xi', uu_uu, a0a1, rho1a[1:]) * 8
    wva[1:] += np.einsum('i,i,xi->xi', uu_ud, a0a1, rho1b[1:]) * 4
    wva[1:] += np.einsum('i,i,xi->xi', uu_ud, a0b1, rho1a[1:]) * 4
    wva[1:] += np.einsum('i,i,xi->xi', ud_ud, a0b1, rho1b[1:]) * 2
    wva[1:] += np.einsum('i,i,xi->xi', uu_ud, b0a1, rho1a[1:]) * 4
    wva[1:] += np.einsum('i,i,xi->xi', uu_dd, b0b1, rho1a[1:]) * 8
    wva[1:] += np.einsum('i,i,xi->xi', ud_ud, b0a1, rho1b[1:]) * 2
    wva[1:] += np.einsum('i,i,xi->xi', ud_dd, b0b1, rho1b[1:]) * 4
    wva[1:] += np.einsum('i,i,xi->xi', uu_uu, a1a1, rho0a[1:]) * 4
    wva[1:] += np.einsum('i,i,xi->xi', uu_ud, a1b1, rho0a[1:]) * 4
    wva[1:] += np.einsum('i,i,xi->xi', uu_dd, b1b1, rho0a[1:]) * 4
    wva[1:] += np.einsum('i,i,xi->xi', uu_ud, a1a1, rho0b[1:]) * 2
    wva[1:] += np.einsum('i,i,xi->xi', ud_ud, a1b1, rho0b[1:]) * 2
    wva[1:] += np.einsum('i,i,xi->xi', ud_dd, b1b1, rho0b[1:]) * 2
    wva[0] += np.einsum('i,i,i->i', u_u_uu, rho1a[0], a0a1) * 4
    wva[0] += np.einsum('i,i,i->i', u_d_uu, rho1b[0], a0a1) * 4
    wva[0] += np.einsum('i,i,i->i', u_u_ud, rho1a[0], a0b1) * 2
    wva[0] += np.einsum('i,i,i->i', u_d_ud, rho1b[0], a0b1) * 2
    wva[0] += np.einsum('i,i,i->i', u_u_ud, rho1a[0], b0a1) * 2
    wva[0] += np.einsum('i,i,i->i', u_d_ud, rho1b[0], b0a1) * 2
    wva[0] += np.einsum('i,i,i->i', u_u_dd, rho1a[0], b0b1) * 4
    wva[0] += np.einsum('i,i,i->i', u_d_dd, rho1b[0], b0b1) * 4
    wva[1:] += np.einsum('i,i,i,xi->xi', u_u_uu, rho1a[0], rho1a[0], rho0a[1:]) * 2
    wva[1:] += np.einsum('i,i,i,xi->xi', u_d_uu, rho1a[0], rho1b[0], rho0a[1:]) * 4
    wva[1:] += np.einsum('i,i,i,xi->xi', d_d_uu, rho1b[0], rho1b[0], rho0a[1:]) * 2
    wva[1:] += np.einsum('i,i,i,xi->xi', u_u_ud, rho1a[0], rho1a[0], rho0a[1:])
    wva[1:] += np.einsum('i,i,i,xi->xi', u_d_ud, rho1a[0], rho1b[0], rho0a[1:]) * 2
    wva[1:] += np.einsum('i,i,i,xi->xi', d_d_ud, rho1b[0], rho1b[0], rho0a[1:])
    wva[1:] += np.einsum('i,i,i,xi->xi', u_uu_uu, rho1a[0], a0a1, rho0a[1:]) * 8
    wva[1:] += np.einsum('i,i,i,xi->xi', d_uu_uu, rho1b[0], a0a1, rho0a[1:]) * 8
    wva[1:] += np.einsum('i,i,i,xi->xi', u_uu_ud, rho1a[0], a0b1, rho0a[1:]) * 4
    wva[1:] += np.einsum('i,i,i,xi->xi', d_uu_ud, rho1b[0], a0b1, rho0a[1:]) * 4
    wva[1:] += np.einsum('i,i,i,xi->xi', u_uu_ud, rho1a[0], b0a1, rho0a[1:]) * 4
    wva[1:] += np.einsum('i,i,i,xi->xi', d_uu_ud, rho1b[0], b0a1, rho0a[1:]) * 4
    wva[1:] += np.einsum('i,i,i,xi->xi', u_uu_dd, rho1a[0], b0b1, rho0a[1:]) * 8
    wva[1:] += np.einsum('i,i,i,xi->xi', d_uu_dd, rho1b[0], b0b1, rho0a[1:]) * 8
    wva[1:] += np.einsum('i,i,i,xi->xi', u_uu_ud, rho1a[0], a0a1, rho0b[1:]) * 4
    wva[1:] += np.einsum('i,i,i,xi->xi', d_uu_ud, rho1b[0], a0a1, rho0b[1:]) * 4
    wva[1:] += np.einsum('i,i,i,xi->xi', u_ud_ud, rho1a[0], a0b1, rho0b[1:]) * 2
    wva[1:] += np.einsum('i,i,i,xi->xi', d_ud_ud, rho1b[0], a0b1, rho0b[1:]) * 2
    wva[1:] += np.einsum('i,i,i,xi->xi', u_ud_ud, rho1a[0], b0a1, rho0b[1:]) * 2
    wva[1:] += np.einsum('i,i,i,xi->xi', d_ud_ud, rho1b[0], b0a1, rho0b[1:]) * 2
    wva[1:] += np.einsum('i,i,i,xi->xi', u_ud_dd, rho1a[0], b0b1, rho0b[1:]) * 4
    wva[1:] += np.einsum('i,i,i,xi->xi', d_ud_dd, rho1b[0], b0b1, rho0b[1:]) * 4
    wva[0] += np.einsum('i,i->i', u_uu_uu, a0a1_a0a1) * 4
    wva[0] += np.einsum('i,i->i', u_uu_ud, a0a1_a0b1) * 2
    wva[0] += np.einsum('i,i->i', u_uu_ud, a0b1_a0a1) * 2
    wva[0] += np.einsum('i,i->i', u_ud_ud, a0b1_a0b1)
    wva[0] += np.einsum('i,i->i', u_uu_ud, a0a1_b0a1) * 4
    wva[0] += np.einsum('i,i->i', u_ud_ud, a0b1_b0a1) * 2
    wva[0] += np.einsum('i,i->i', u_uu_dd, a0a1_b0b1) * 8
    wva[0] += np.einsum('i,i->i', u_ud_dd, a0b1_b0b1) * 4
    wva[0] += np.einsum('i,i->i', u_ud_ud, b0a1_b0a1)
    wva[0] += np.einsum('i,i->i', u_ud_dd, b0a1_b0b1) * 2
    wva[0] += np.einsum('i,i->i', u_ud_dd, b0b1_b0a1) * 2
    wva[0] += np.einsum('i,i->i', u_dd_dd, b0b1_b0b1) * 4
    wva[1:] += np.einsum('i,i,xi->xi', uu_uu_uu, a0a1_a0a1, rho0a[1:]) * 8
    wva[1:] += np.einsum('i,i,xi->xi', uu_uu_ud, a0a1_a0b1, rho0a[1:]) * 4
    wva[1:] += np.einsum('i,i,xi->xi', uu_uu_ud, a0b1_a0a1, rho0a[1:]) * 4
    wva[1:] += np.einsum('i,i,xi->xi', uu_ud_ud, a0b1_a0b1, rho0a[1:]) * 2
    wva[1:] += np.einsum('i,i,xi->xi', uu_uu_ud, a0a1_a0a1, rho0b[1:]) * 4
    wva[1:] += np.einsum('i,i,xi->xi', uu_ud_ud, a0a1_a0b1, rho0b[1:]) * 2
    wva[1:] += np.einsum('i,i,xi->xi', uu_ud_ud, a0b1_a0a1, rho0b[1:]) * 2
    wva[1:] += np.einsum('i,i,xi->xi', ud_ud_ud, a0b1_a0b1, rho0b[1:])
    wva[1:] += np.einsum('i,i,xi->xi', uu_uu_ud, a0a1_b0a1, rho0a[1:]) * 8
    wva[1:] += np.einsum('i,i,xi->xi', uu_ud_ud, a0b1_b0a1, rho0a[1:]) * 4
    wva[1:] += np.einsum('i,i,xi->xi', uu_uu_dd, a0a1_b0b1, rho0a[1:]) * 16
    wva[1:] += np.einsum('i,i,xi->xi', uu_ud_dd, a0b1_b0b1, rho0a[1:]) * 8
    wva[1:] += np.einsum('i,i,xi->xi', uu_ud_ud, a0a1_b0a1, rho0b[1:]) * 4
    wva[1:] += np.einsum('i,i,xi->xi', ud_ud_ud, a0b1_b0a1, rho0b[1:]) * 2
    wva[1:] += np.einsum('i,i,xi->xi', uu_ud_dd, a0a1_b0b1, rho0b[1:]) * 8
    wva[1:] += np.einsum('i,i,xi->xi', ud_ud_dd, a0b1_b0b1, rho0b[1:]) * 4
    wva[1:] += np.einsum('i,i,xi->xi', uu_ud_ud, b0a1_b0a1, rho0a[1:]) * 2
    wva[1:] += np.einsum('i,i,xi->xi', uu_ud_dd, b0b1_b0a1, rho0a[1:]) * 4
    wva[1:] += np.einsum('i,i,xi->xi', uu_ud_dd, b0a1_b0b1, rho0a[1:]) * 4
    wva[1:] += np.einsum('i,i,xi->xi', uu_dd_dd, b0b1_b0b1, rho0a[1:]) * 8
    wva[1:] += np.einsum('i,i,xi->xi', ud_ud_ud, b0a1_b0a1, rho0b[1:])
    wva[1:] += np.einsum('i,i,xi->xi', ud_ud_dd, b0b1_b0a1, rho0b[1:]) * 2
    wva[1:] += np.einsum('i,i,xi->xi', ud_ud_dd, b0a1_b0b1, rho0b[1:]) * 2
    wva[1:] += np.einsum('i,i,xi->xi', ud_dd_dd, b0b1_b0b1, rho0b[1:]) * 4
    wva *= weight
    wva[0]*=.5  # v+v.T should be applied in the caller

    wvb = np.zeros((4,ngrid))
    wvb[0] += np.einsum('i,i,i->i', d_d_d, rho1b[0], rho1b[0])
    wvb[0] += np.einsum('i,i,i->i', u_d_d, rho1b[0], rho1a[0]) * 2
    wvb[0] += np.einsum('i,i,i->i', u_u_d, rho1a[0], rho1a[0])
    wvb[0] += np.einsum('i,i->i', d_dd, b1b1) * 2
    wvb[0] += np.einsum('i,i->i', d_ud, b1a1) * 2
    wvb[0] += np.einsum('i,i->i', d_uu, a1a1) * 2
    wvb[1:] += np.einsum('i,i,xi->xi', d_dd, rho1b[0], rho1b[1:]) * 4
    wvb[1:] += np.einsum('i,i,xi->xi', u_dd, rho1a[0], rho1b[1:]) * 4
    wvb[1:] += np.einsum('i,i,xi->xi', d_ud, rho1b[0], rho1a[1:]) * 2
    wvb[1:] += np.einsum('i,i,xi->xi', u_ud, rho1a[0], rho1a[1:]) * 2
    wvb[1:] += np.einsum('i,i,xi->xi', dd_dd, b0b1, rho1b[1:]) * 8
    wvb[1:] += np.einsum('i,i,xi->xi', ud_dd, b0b1, rho1a[1:]) * 4
    wvb[1:] += np.einsum('i,i,xi->xi', ud_dd, b0a1, rho1b[1:]) * 4
    wvb[1:] += np.einsum('i,i,xi->xi', ud_ud, b0a1, rho1a[1:]) * 2
    wvb[1:] += np.einsum('i,i,xi->xi', ud_dd, a0b1, rho1b[1:]) * 4
    wvb[1:] += np.einsum('i,i,xi->xi', uu_dd, a0a1, rho1b[1:]) * 8
    wvb[1:] += np.einsum('i,i,xi->xi', ud_ud, a0b1, rho1a[1:]) * 2
    wvb[1:] += np.einsum('i,i,xi->xi', uu_ud, a0a1, rho1a[1:]) * 4
    wvb[1:] += np.einsum('i,i,xi->xi', dd_dd, b1b1, rho0b[1:]) * 4
    wvb[1:] += np.einsum('i,i,xi->xi', ud_dd, b1a1, rho0b[1:]) * 4
    wvb[1:] += np.einsum('i,i,xi->xi', uu_dd, a1a1, rho0b[1:]) * 4
    wvb[1:] += np.einsum('i,i,xi->xi', ud_dd, b1b1, rho0a[1:]) * 2
    wvb[1:] += np.einsum('i,i,xi->xi', ud_ud, b1a1, rho0a[1:]) * 2
    wvb[1:] += np.einsum('i,i,xi->xi', uu_ud, a1a1, rho0a[1:]) * 2
    wvb[0] += np.einsum('i,i,i->i', d_d_dd, rho1b[0], b0b1) * 4
    wvb[0] += np.einsum('i,i,i->i', u_d_dd, rho1a[0], b0b1) * 4
    wvb[0] += np.einsum('i,i,i->i', d_d_ud, rho1b[0], b0a1) * 2
    wvb[0] += np.einsum('i,i,i->i', u_d_ud, rho1a[0], b0a1) * 2
    wvb[0] += np.einsum('i,i,i->i', d_d_ud, rho1b[0], a0b1) * 2
    wvb[0] += np.einsum('i,i,i->i', u_d_ud, rho1a[0], a0b1) * 2
    wvb[0] += np.einsum('i,i,i->i', d_d_uu, rho1b[0], a0a1) * 4
    wvb[0] += np.einsum('i,i,i->i', u_d_uu, rho1a[0], a0a1) * 4
    wvb[1:] += np.einsum('i,i,i,xi->xi', d_d_dd, rho1b[0], rho1b[0], rho0b[1:]) * 2
    wvb[1:] += np.einsum('i,i,i,xi->xi', u_d_dd, rho1b[0], rho1a[0], rho0b[1:]) * 4
    wvb[1:] += np.einsum('i,i,i,xi->xi', u_u_dd, rho1a[0], rho1a[0], rho0b[1:]) * 2
    wvb[1:] += np.einsum('i,i,i,xi->xi', d_d_ud, rho1b[0], rho1b[0], rho0b[1:])
    wvb[1:] += np.einsum('i,i,i,xi->xi', u_d_ud, rho1b[0], rho1a[0], rho0b[1:]) * 2
    wvb[1:] += np.einsum('i,i,i,xi->xi', u_u_ud, rho1a[0], rho1a[0], rho0b[1:])
    wvb[1:] += np.einsum('i,i,i,xi->xi', d_dd_dd, rho1b[0], b0b1, rho0b[1:]) * 8
    wvb[1:] += np.einsum('i,i,i,xi->xi', u_dd_dd, rho1a[0], b0b1, rho0b[1:]) * 8
    wvb[1:] += np.einsum('i,i,i,xi->xi', d_ud_dd, rho1b[0], b0a1, rho0b[1:]) * 4
    wvb[1:] += np.einsum('i,i,i,xi->xi', u_ud_dd, rho1a[0], b0a1, rho0b[1:]) * 4
    wvb[1:] += np.einsum('i,i,i,xi->xi', d_ud_dd, rho1b[0], a0b1, rho0b[1:]) * 4
    wvb[1:] += np.einsum('i,i,i,xi->xi', u_ud_dd, rho1a[0], a0b1, rho0b[1:]) * 4
    wvb[1:] += np.einsum('i,i,i,xi->xi', d_uu_dd, rho1b[0], a0a1, rho0b[1:]) * 8
    wvb[1:] += np.einsum('i,i,i,xi->xi', u_uu_dd, rho1a[0], a0a1, rho0b[1:]) * 8
    wvb[1:] += np.einsum('i,i,i,xi->xi', d_ud_dd, rho1b[0], b0b1, rho0a[1:]) * 4
    wvb[1:] += np.einsum('i,i,i,xi->xi', u_ud_dd, rho1a[0], b0b1, rho0a[1:]) * 4
    wvb[1:] += np.einsum('i,i,i,xi->xi', d_ud_ud, rho1b[0], b0a1, rho0a[1:]) * 2
    wvb[1:] += np.einsum('i,i,i,xi->xi', u_ud_ud, rho1a[0], b0a1, rho0a[1:]) * 2
    wvb[1:] += np.einsum('i,i,i,xi->xi', d_ud_ud, rho1b[0], a0b1, rho0a[1:]) * 2
    wvb[1:] += np.einsum('i,i,i,xi->xi', u_ud_ud, rho1a[0], a0b1, rho0a[1:]) * 2
    wvb[1:] += np.einsum('i,i,i,xi->xi', d_uu_ud, rho1b[0], a0a1, rho0a[1:]) * 4
    wvb[1:] += np.einsum('i,i,i,xi->xi', u_uu_ud, rho1a[0], a0a1, rho0a[1:]) * 4
    wvb[0] += np.einsum('i,i->i', d_dd_dd, b0b1_b0b1) * 4
    wvb[0] += np.einsum('i,i->i', d_ud_dd, b0b1_b0a1) * 2
    wvb[0] += np.einsum('i,i->i', d_ud_dd, b0a1_b0b1) * 2
    wvb[0] += np.einsum('i,i->i', d_ud_ud, b0a1_b0a1)
    wvb[0] += np.einsum('i,i->i', d_ud_dd, b0b1_a0b1) * 4
    wvb[0] += np.einsum('i,i->i', d_ud_ud, b0a1_a0b1) * 2
    wvb[0] += np.einsum('i,i->i', d_uu_dd, b0b1_a0a1) * 8
    wvb[0] += np.einsum('i,i->i', d_uu_ud, b0a1_a0a1) * 4
    wvb[0] += np.einsum('i,i->i', d_ud_ud, a0b1_a0b1)
    wvb[0] += np.einsum('i,i->i', d_uu_ud, a0b1_a0a1) * 2
    wvb[0] += np.einsum('i,i->i', d_uu_ud, a0a1_a0b1) * 2
    wvb[0] += np.einsum('i,i->i', d_uu_uu, a0a1_a0a1) * 4
    wvb[1:] += np.einsum('i,i,xi->xi', dd_dd_dd, b0b1_b0b1, rho0b[1:]) * 8
    wvb[1:] += np.einsum('i,i,xi->xi', ud_dd_dd, b0b1_b0a1, rho0b[1:]) * 4
    wvb[1:] += np.einsum('i,i,xi->xi', ud_dd_dd, b0a1_b0b1, rho0b[1:]) * 4
    wvb[1:] += np.einsum('i,i,xi->xi', ud_ud_dd, b0a1_b0a1, rho0b[1:]) * 2
    wvb[1:] += np.einsum('i,i,xi->xi', ud_dd_dd, b0b1_b0b1, rho0a[1:]) * 4
    wvb[1:] += np.einsum('i,i,xi->xi', ud_ud_dd, b0b1_b0a1, rho0a[1:]) * 2
    wvb[1:] += np.einsum('i,i,xi->xi', ud_ud_dd, b0a1_b0b1, rho0a[1:]) * 2
    wvb[1:] += np.einsum('i,i,xi->xi', ud_ud_ud, b0a1_b0a1, rho0a[1:])
    wvb[1:] += np.einsum('i,i,xi->xi', ud_dd_dd, b0b1_a0b1, rho0b[1:]) * 8
    wvb[1:] += np.einsum('i,i,xi->xi', ud_ud_dd, b0a1_a0b1, rho0b[1:]) * 4
    wvb[1:] += np.einsum('i,i,xi->xi', uu_dd_dd, b0b1_a0a1, rho0b[1:]) * 16
    wvb[1:] += np.einsum('i,i,xi->xi', uu_ud_dd, b0a1_a0a1, rho0b[1:]) * 8
    wvb[1:] += np.einsum('i,i,xi->xi', ud_ud_dd, b0b1_a0b1, rho0a[1:]) * 4
    wvb[1:] += np.einsum('i,i,xi->xi', ud_ud_ud, b0a1_a0b1, rho0a[1:]) * 2
    wvb[1:] += np.einsum('i,i,xi->xi', uu_ud_dd, b0b1_a0a1, rho0a[1:]) * 8
    wvb[1:] += np.einsum('i,i,xi->xi', uu_ud_ud, b0a1_a0a1, rho0a[1:]) * 4
    wvb[1:] += np.einsum('i,i,xi->xi', ud_ud_dd, a0b1_a0b1, rho0b[1:]) * 2
    wvb[1:] += np.einsum('i,i,xi->xi', uu_ud_dd, a0a1_a0b1, rho0b[1:]) * 4
    wvb[1:] += np.einsum('i,i,xi->xi', uu_ud_dd, a0b1_a0a1, rho0b[1:]) * 4
    wvb[1:] += np.einsum('i,i,xi->xi', uu_uu_dd, a0a1_a0a1, rho0b[1:]) * 8
    wvb[1:] += np.einsum('i,i,xi->xi', ud_ud_ud, a0b1_a0b1, rho0a[1:])
    wvb[1:] += np.einsum('i,i,xi->xi', uu_ud_ud, a0a1_a0b1, rho0a[1:]) * 2
    wvb[1:] += np.einsum('i,i,xi->xi', uu_ud_ud, a0b1_a0a1, rho0a[1:]) * 2
    wvb[1:] += np.einsum('i,i,xi->xi', uu_uu_ud, a0a1_a0a1, rho0a[1:]) * 4
    wvb *= weight
    wvb[0]*=.5
...
def get_rho(ni, mol, dm, grids, max_memory=2000):
    ...
    assert(nset == 1)
    rho = np.empty(grids.weights.size)
    p1 = 0
    for ao, mask, weight, coords \
...
class NumInt(object):
    ...
    def nr_vxc(self, mol, grids, xc_code, dms, spin=0, relativity=0, hermi=0,
               max_memory=2000, verbose=None):
        if spin == 0:
            return self.nr_rks_sep(mol, grids, xc_code, dms, relativity, hermi,
                               max_memory, verbose)
        else:
