...
def gen_partition_sep(mol, atom_grids_tab,
                  radii_adjust=None, atomic_radii=radi.BRAGG_RADII,
                  becke_scheme=original_becke):
    '''Generate the mesh grid coordinates and weights for DFT numerical integration.
    We can change radii_adjust, becke_scheme functions to generate different meshgrid.
    ...
    for ia in range(mol.natm):
        ...
        coords_all.append(coords)
        weights_all.append(weights)
        nums_grids.append(len(weights))
        #print( coords.shape, weights.shape )
    return numpy.vstack(coords_all), numpy.hstack(weights_all), nums_grids
...
# don't modify the following attributes, they are not input options
    ...
    def build(self, mol=None, with_non0tab=False, **kwargs):
        ...
        self.coords, self.weights, self.nums_grids = \
                self.gen_partition_sep(mol, atom_grids_tab,
    ...
    @lib.with_doc(gen_partition_sep.__doc__)
    def gen_partition_sep(self, mol, atom_grids_tab,
                      radii_adjust=None, atomic_radii=radi.BRAGG_RADII,
                      becke_scheme=original_becke):
        ''' See gen_grid.gen_partition_sep function'''
        return gen_partition_sep(mol, atom_grids_tab, radii_adjust, atomic_radii,
                             becke_scheme)
