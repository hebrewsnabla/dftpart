...
!  f2py -m frame_small6 -c frame_small6.f90  -L/home/liaokang/anaconda3/lib/python3.6/site-packages/pyscf/lib/ -lcint --fcompiler=intelem --compiler=intelem -liomp5
! -----------------------------------------------------------------------
...
!f2py intent(out) :: atom_ej
!f2py intent(out) :: atom_ek
!f2py depend(nao) :: dm
!f2py depend(basl) :: bas
!f2py depend(envl) :: env
!f2py depend(atml) :: atm

!f2py depend(singleitem) :: atom_ej
!f2py depend(singleitem) :: atom_ek
          ...
          do kbas = kfi,kls
             ...
             do l=kbas,nao
                ...
                if (dabs(e_coul) > 1D-12) then
                  ...
                  atom_ej(II) = atom_ej(II) + e_coul_j 
                  atom_ej(JJ) = atom_ej(JJ) + e_coul_j
                  atom_ej(KK) = atom_ej(KK) + e_coul_j
                  atom_ej(LL) = atom_ej(LL) + e_coul_j

                  atom_ek(II) = atom_ek(II) + e_coul_k 
                  atom_ek(JJ) = atom_ek(JJ) + e_coul_k
                  atom_ek(KK) = atom_ek(KK) + e_coul_k
                  atom_ek(LL) = atom_ek(LL) + e_coul_k
                endif
          enddo
        enddo
      enddo
