...
def get_E1(eda, atm2bas):
    ...
    with open(eda.output+'-eda.log','a') as f:
        logger.log(f,"atom_kinE=",atom_kin)
    ...
    with open(eda.output+'-eda.log','a') as f:
        logger.log(f,"atom_1enuc=",atom_1enuc)
    atm_e1 = atom_kin + atom_1enuc
    with open(eda.output+'-eda.log','a') as f:
        logger.log(f,"atom_e1=",atm_e1)
        if eda.verbose > 5:
            ...
            logger.mlog(f,"kin_err: ",kin_err)
            logger.mlog(f,"1enuc_err: ",a1enuc_err)
...
def get_Enuc(eda):
    ...
    with open(eda.output+'-eda.log','a') as f:
        logger.log(f,"atom_enucnuc=",atm_enucnuc)
        if eda.verbose > 5:
