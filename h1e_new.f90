...
subroutine bgh1e_inter(dm,bas2atm,bas2frg, vchg_frag,natm,nao,nfrag,energy_ebg, bg2, bg3)
implicit none
...
!integer atm2frg(natm)
real*8 dm(nao,nao)
real*8 vchg_frag(nao,nao)
real*8 energy_ebg(natm)
!real*8 e1_1(nfrag)
real*8 bg2(nfrag)
real*8 bg3(nfrag,nfrag)
!real*8 e1_3(natm,natm,natm)
real*8 tem
real*8 H1E_THRESH
...
!f2py intent(in) :: natm,nao,nfrag
!f2py intent(out) :: energy_ebg, bg2, bg3

!f2py depend(natm) :: atm2frg, energy_ebg,
!f2py depend(nao) :: dm, bas2atm, bas2frg, vchg_frag
!f2py depend(nfrag) :: bg2, bg3
...
!$OMP PARALLEL DO schedule(guided) &
!$omp default(private) &
!$omp shared(dm,bas2atm,bas2frg, vchg_frag,nao,natm,nfrag, H1E_THRESH) &
!$omp reduction(+:energy_ebg, bg2, bg3)
do i=1,nao 
  a = bas2atm(i)+1
  aa = bas2frg(i)
  do j=1,nao 
    ...
    !  ll = atm2frg(l)
      !write(*,*) aa,bb,ll
      tem = dm(i,j) * vchg_frag(j,i)
      !if (dabs(tem) > H1E_THRESH) then 
      if (.true.) then
          !energy_h1e(l) = energy_h1e(l) + tem*1/2.0d0
          energy_ebg(a) = energy_ebg(a) + tem*1/4.0d0
          energy_ebg(b) = energy_ebg(b) + tem*1/4.0d0
          ...
          if (aa==bb) then
            ...
            !else
              !write(*,*) aa,bb,ll,tem
              bg3(aa,bb) = bg3(aa,bb) + tem
              !e1_3(aa,ll,bb) = e1_3(aa,ll,bb) + tem
              bg3(bb,aa) = bg3(bb,aa) + tem
              !e1_3(bb,ll,aa) = e1_3(bb,ll,aa) + tem
              !e1_3(ll,aa,bb) = e1_3(ll,aa,bb) + tem
              !e1_3(ll,bb,aa) = e1_3(ll,bb,aa) + tem
            !endif
